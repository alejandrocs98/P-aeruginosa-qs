...
params = (N, k_lasR, g_lasR, k_LasR, g_LasR, a_rsaL, b_rsaL, K1, h1, g_rsaL, k_RsaL, g_RsaL, a_lasI, b_lasI, \
...
def solve_qs(qs_sys=LasRI_RsaL_qs, t_span=t_span, r0=r0, params=params):
    return solve_ivp(lambda t, r: qs_sys(t, r, params), t_span, r0)
...
def plot_det2(qs_dynamics_short, qs_dynamics_long, N=N, action='display'):
    ...
    plt.subplot(1,2,1)
    for i in range(len(x)):
        plt.plot(qs_dynamics_short.t, qs_dynamics_short.y[i], lw=3, label=x[i], color=cm.tab10(i), alpha=0.7)
    plt.grid()
    plt.legend(loc='center right', bbox_to_anchor=(1.20, 0.50), ncol=1, fancybox=True, shadow=False, fontsize=12)
    plt.xlabel('Time', fontsize=12)
    plt.ylabel('Concentration', fontsize=12)
    ...
    plt.subplot(1,2,2)
    for i in range(len(x)):
        plt.plot(qs_dynamics_long.t, qs_dynamics_long.y[i], lw=3, label=x[i], color=cm.tab10(i), alpha=0.7)
    plt.grid()
    plt.legend(loc='center right', bbox_to_anchor=(1.20, 0.50), ncol=1, fancybox=True, shadow=False, fontsize=12)
    plt.xlabel('Time', fontsize=12)
    plt.ylabel('Concentration', fontsize=12)
    ...
    if action == 'save':
        fig.savefig(f'qs_dynamics2_{N}.png')
    else:
        return fig